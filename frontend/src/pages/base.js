...
const BaseLayout = () => {
  ...
  const dragStart = (event) => {
    ...
    if (
      typeof activeIdLocal === "string" &&
      activeIdLocal.startsWith("df-item-")
    ) {
      ...
      if (parentDeepFeed?.feeds) {
        const content = parentDeepFeed.feeds.find(
          (item) => item.feed?.feed_id === feedId
        );
        if (content) {
          dragItemLocal = { feed: content.feed, parentDeepFeedId };
        } else {
          for (const df of deepFeeds) {
            const fallback = df.feeds?.find(
              (item) => item.feed?.feed_id === feedId
            );
            if (fallback) {
              dragItemLocal = {
                feed: fallback.feed,
                parentDeepFeedId: df.deep_feed_id,
              };
              break;
            }
          }
      ...
      typeof activeIdLocal === "string" &&
      activeIdLocal.startsWith("nested-df-")
    ) {
    ...
    if (!over) {
      if (dragType === "feedInDeepFeed" && activeDragItem) {
        try {
          ...
          if (
            data.success &&
            deepFeedCallbacks[activeDragItem.parentDeepFeedId]
          ) {
            deepFeedCallbacks[activeDragItem.parentDeepFeedId]({
    ...
    try {
      ...
      if (
        ...
        const sourceFeed = feeds.find(
          (f) => f.feed_id.toString() === activeIdLocal
        );
                ...
                setDeepFeeds((prev) =>
                  prev.map((df) => {
                    if (df.deep_feed_id === sourceParentDeepFeedId) {
                      const filtered = (df.feeds || []).filter(
                        (item) =>
                          !(
                            item.feed &&
                      ...
                      return {
                        ...df,
                        feeds: [...filtered, { nestedDeepFeed: data.deepFeed }],
                      };
                    }
                    return df;
                  })
