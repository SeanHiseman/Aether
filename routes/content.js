...
router.get("/channel_posts", async (req, res) => {
  try {
    const { channelId, feedId, isMain, isSingle, limit, offset, postId } =
      req.query;
...
const postFilter = (req, file, cb) => {
  ...
  if (file.size > maxSize) {
    return cb(
      new Error(`File exceeds the limit of ${maxSize / (1024 * 1024)}MB`)
    );
  }
  cb(null, true);
};
...
router.post(
  ...
  async (req, res) => {
    try {
        ...
        $('img[src^="blob:"], video[src^="blob:"]').each((i, el) => {
          if (index < req.files.length) {
            ...
            if (fileType === "img") {
              ...
              $(el).empty();
              $(el).append(
                `<source src="/media/content/${file.filename}" type="${file.mimetype}">`
              );
            }
            index++;
          }
      ...
      if (parent_id) {
        //parent_id means post is a reply
...
const deleteBuilds = async (html) => {
  ...
  let match;
  while ((match = regexPath.exec(html)) !== null)
...
router.get("/post_replies/:postId", async (req, res) => {
  try {
    ...
    if (!parentPost) {
      return res
        .status(404)
        .json({ success: false, message: "Parent post not found." });
    }
    const includeOptions = [
      {
        model: Feeds,
    ...
    yauzl.open(filePath, { lazyEntries: true }, (err, zipfile) => {
      ...
      zipfile.on("entry", (entry) => {
        ...
        if (
          ...
          totalUncompressed > MAX_TOTAL_UNCOMPRESSED
        ) {
          zipfile.close();
...
const buildFilter = (req, file, cb) => {
  ...
  return ok ? cb(null, true) : cb(new Error("Only .zip builds are allowed"));
};
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      await AppBuilds.create({
        build_id: buildId,
        path: `/app_builds/${buildId}`,
      });
...
router.get("/explore_posts", async (req, res) => {
  try {
    ...
    // Random order: Sequelize's RANDOM() for sqlite/postgres, RAND() for MySQL
    const posts = await Posts.findAll({
      attributes: postAttributes,
      include: [
        {
      ...
      where, // This now correctly filters out replies
      limit: parseInt(limit, 10),
