...
const defaultImages = [
  ...
  process.env.DEFAULT_GROUP_IMAGE,
];
...
router.post("/accept_follow_request", authenticateCheck, async (req, res) => {
  try {
    ...
    await Feeds.increment("follower_count", {
      where: { feed_id: follow_request.receiver_id },
    });
...
router.post("/add_feed_channel", authenticateCheck, async (req, res) => {
  ...
  try {
    ...
    const maxOrderResult = await FeedChannels.findOne({
      attributes: [
        [
          sequelize.fn(
            "COALESCE",
            sequelize.fn("MAX", sequelize.col("display_order")),
            -1
          ),
          "maxOrder",
        ],
    ...
    const nextDisplayOrder =
      (maxOrderResult ? maxOrderResult.maxOrder : -1) + 1;
    ...
    const newChannel = await FeedChannels.create(newChannelData, {
      transaction,
    });
  ...
  try {
    ...
    if (!feedId && !nestedDeepFeedId) {
      return res.status(400).json({
        success: false,
        message: "Must provide either a feedId or nestedDeepFeedId",
      });
    }
    if (nestedDeepFeedId) {
...
router.post("/change_channel_name", authenticateCheck, async (req, res) => {
  try {
    ...
    if (newChannelName === "Main") {
      res
        .status(401)
        .json({ success: false, message: "Channel can't be called main" });
    } else {
...
router.post("/change_deep_feed_name", authenticateCheck, async (req, res) => {
  try {
    ...
    if (newName === "Following") {
      res
        .status(401)
        .json({ success: false, message: "Can't be called Following" });
    } else {
      await DeepFeeds.update(
        { name: newName },
    ...
    feedProfileUpload(req, res, async function (error) {
      if (error instanceof multer.MulterError) {
        ...
        if (error.code === "LIMIT_FILE_SIZE") {
          return res
            .status(413)
            .json({ error: "File cannot be more than 100MB" });
        }
      ...
      try {
        ...
        if (existingFeed) {
          if (req.file) {
            fs.unlinkSync(
              path.join(process.cwd(), "/media/feed_images", req.file.filename)
            );
          }
...
router.post("/create_deep_feed", authenticateCheck, async (req, res) => {
  try {
    const { deepFeedName, feedsToInclude, parentDeepFeedId, viewerId } =
      req.body;
    console.log(
      "Creating deep feed:",
      "deepFeedName:",
    ...
    if (!feedsToInclude || feedsToInclude.length < 2) {
      return res
        .status(400)
        .json({ success: false, message: "At least two feeds are required" });
    }
...
router.get("/deep_feed_posts", authenticateCheck, async (req, res) => {
  try {
    ...
    if (deepFeedId === "following") {
      //Following deep feed is a special case
      ...
      const getAllFeedIdsInDeepFeed = async (
        deepFeedId,
        visited = new Set()
      ) => {
        ...
        for (const content of contents) {
          ...
          if (content.nested_deep_feed_id) {
            const nested = await getAllFeedIdsInDeepFeed(
              content.nested_deep_feed_id,
              visited
            );
            feedIds.push(...nested);
          }
...
router.delete("/delete_feed", authenticateCheck, async (req, res) => {
  ...
  try {
    ...
    if (feedPostIds.length > 0) {
      await PostNotes.destroy({
        where: { post_id: { [Op.in]: feedPostIds } },
        transaction,
      });
    ...
    await FollowRequests.destroy({
      where: {
        [Op.or]: [{ receiver_id: feedId }, { sender_id: feedId }],
      },
      transaction,
    });
...
router.delete("/delete_feed_channel", authenticateCheck, async (req, res) => {
  ...
  try {
    ...
    const deletedCount = await FeedChannels.destroy({
      where: { channel_id: channelId },
      transaction,
    });
    if (deletedCount === 0) {
  ...
  try {
    ...
    const channels = await FeedChannels.findAll({
      ...
      order: [
        ["display_order", "ASC"],
        ["channel_name", "ASC"],
      ], //Secondary in case of same display order
    });
...
router.post("/remove_from_deep_feed", authenticateCheck, async (req, res) => {
  try {
    ...
    const where = {
      ...
      ...(feedId
        ? { feed_id: feedId }
        : { nested_deep_feed_id: nestedDeepFeedId }),
    };
...
router.put("/reorder_feed_channels", async (req, res) => {
  ...
  try {
    ...
    if (!feed_id || !Array.isArray(orderedChannelIds)) {
      return res.status(400).json({
        success: false,
        message: "Missing feed_id or orderedChannelIds.",
      });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Channels reordered successfully." });
  } catch (error) {
    if (transaction) await transaction.rollback();
    res.status(500).json({
      success: false,
    ...
    feedProfileUpload(req, res, async function (error) {
      if (error instanceof multer.MulterError) {
        ...
        if (error.code === "LIMIT_FILE_SIZE") {
          return res
            .status(413)
            .json({ error: "File cannot be more than 5MB" });
        }
      ...
      try {
        ...
        if (user.storage_count + fileSize > maxStorage) {
          fs.unlinkSync(
            path.join(process.cwd(), "/media/feed_images", file.filename)
          );
          return res.status(413).json({
            success: false,
            message: `Weekly limit of ${maxStorage}MB exceeded`,
  ...
  socket.on("send_feed_message", async (message) => {
    try {
      ...
      socket
        ...
        .emit("channel_message_confirmed", newMessage);
    } catch (err) {
