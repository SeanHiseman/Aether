...
const SortableFeedChannelItem = ({ channel, id, url }) => {
	...
	return (
		<li
			ref={setNodeRef}
			style={style}
			className={`channel-item ${isDragging ? 'dragging-active' : ''} ${isSelected ? 'selected' : ''}`}
			{...attributes}
			{...listeners}
		>
...
const ChannelList = ({ canReorder = false, channels, feedId, feedName, isChat, isGroup, isSaved, setChannels }) => {
	...
	const mainChannel = currentChannels.find(channel => channel?.channel_name === 'Main' || channel?.title === 'Main');
	const nonMainChannels = currentChannels
		.filter(channel => channel?.channel_name !== 'Main' && channel?.title !== 'Main')
		.sort((a, b) => new Date(b?.updated_at || 0) - new Date(a?.updated_at || 0));

	const orderedChannels = mainChannel ? [mainChannel, ...nonMainChannels] : nonMainChannels;
					...
					<SortableContext items={channelIdsForDnd} strategy={verticalListSortingStrategy}>
						<nav className="channel-list">
							...
							<ul>
								{mainChannel && (() => {
									const url = isSaved
										? `/saved/${mainChannel?.channel_name}`
										: `/${urlLetter}/${feedName}/${mainChannel?.channel_name}`;
									const isSelected = location.pathname === url;

									return (
										<li key={mainChannel?.channel_id} className={`channel-item ${isSelected ? 'selected' : ''}`}>
											<Link to={url}>
												<div className={`channel-link ${isSelected ? 'selected' : ''}`}>
	...
	return (
		<nav className="channel-list">
			<p className="channel-header-text" style={{ margin: 0 }}>Chats</p>
			{errorMessage && <div className="error-message">{errorMessage}</div>}
			<ul>
				{orderedChannels.map(channel => (
