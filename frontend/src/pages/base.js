...
const BaseLayout = () => {
  ...
  const dragStart = (event) => {
    ...
    if (typeof activeId === "string" && activeId.startsWith("df-item-")) {
      ...
      const parentDeepFeed = deepFeeds.find(
        (df) => df.deep_feed_id === parentDeepFeedId
      );
      if (parentDeepFeed && parentDeepFeed.feeds) {
        const content = parentDeepFeed.feeds.find(
          (item) => item.feed && item.feed.feed_id === feedId
        );
        if (content) {
          dragItem = {
            feed: content.feed,
          ...
          for (const deepFeed of deepFeeds) {
            if (deepFeed.feeds) {
              const fallbackContent = deepFeed.feeds.find(
                (item) => item.feed && item.feed.feed_id === feedId
              );
              if (fallbackContent) {
                dragItem = {
    ...
    if (!dragItem && dragType === "feedInDeepFeed") {
      console.warn(
        "Failed to find feed item using standard methods, trying alternatives"
      );
      const feedId = activeId.replace("df-item-", "");
      const feedFromAllFeeds = feeds.find(
        (feed) => feed.feed_id.toString() === feedId
      );
      if (feedFromAllFeeds) {
        dragItem = {
          feed: {
  ...
  const dragEnd = async (event) => {
      ...
      if (dragType === "feedInDeepFeed") {
        ...
        try {
          ...
          if (data.success && deepFeedCallbacks[sourceParentDeepFeedId]) {
            deepFeedCallbacks[sourceParentDeepFeedId]({
    ...
    try {
      ...
      if (
        ...
        console.log("case 1");
        const sourceFeed = feeds.find(
          (feed) => feed.feed_id.toString() === activeId
        );
        const destinationFeed = feeds.find(
          (feed) => feed.feed_id.toString() === overId
        );
            ...
            if (data.success) {
              if (deepFeedCallbacks[targetParentDeepFeedId]) {
                deepFeedCallbacks[targetParentDeepFeedId](
                  sourceFeed.followedFeed
                ); //DeepFeedItem uses data from followedFeed
              } else {
      ...
      //Case 3: Creating a nested deep feed (two feeds dragged and dropped together in same deep feed)
        ...
        if (deepFeedName) {
          try {
            ...
            if (data.success && data.deepFeed) {
              const parentDeepFeed = deepFeeds.find(
                (df) => df.deep_feed_id === sourceParentDeepFeedId
              );
      ...
      //Case 4: Moving feed from one deep feed to another
        ...
        try {
          ...
          if (addData.success) {
            const { data: removeData } = await axios.post(
              "/api/remove_from_deep_feed",
              {
                deepFeedId: sourceParentDeepFeedId,
                feedId: sourceFeedId,
              }
          ...
          {isAuthenticated ? (
            ...
            >
              ...
              <div className="deep-feeds-container">
                ...
                <SortableContext
                  items={deepFeeds.map((df) => `df-${df.deep_feed_id}`)}
                ...
                {showForm && (
                  <form id="create-feed-form" onSubmit={createFeed}>
                    ...
                    <div className="option-toggle">
                      <button
                        className={
                          feedType === "public" ? "active-mode" : "passive-mode"
                        }
                        onClick={(event) => {
                          event.preventDefault();
                          setFeedType("public");
              ...
              <nav className="feed-list">
                <SortableContext
                  items={feeds.map((feed) => feed.feed_id.toString())}
                  strategy={verticalListSortingStrategy}
                >
        ...
        <main>
          <header className="base-header">
            <button
              className="small-icon"
              onClick={toggleLeft}
              title={desk.left ? "Open sidebar" : "Close sidebar"}
            >
            ...
            <button
              className="small-icon"
              onClick={toggleRight}
              title={desk.right ? "Open sidebar" : "Close sidebar"}
            >
