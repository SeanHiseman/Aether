...
import { Posts, SavedPosts } from "../models/relationships.js";
...
async function ApplyAlgorithm({ channelId, excludedPostIds, feedId, includeOptions, isMain, limit, offset, saverId, userId }) {
    const excludedIds = excludedPostIds ? excludedPostIds.split(',') : [];
    let algorithm = {};
    let algorithmLocation = null;
    if (userId && channelId) {
        algorithmLocation = await AlgorithmLocations.findOne({ where: { location_id: channelId, user_id: userId } });
